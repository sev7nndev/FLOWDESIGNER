...
import { History, Sparkles, Share2, Copy, X } from 'lucide-react';
...
const ResultDisplayComponent: React.FC<ResultDisplayProps> = ({ state, downloadImage, showGallery, setShowGallery, onDelete }) => {
                    ...
                    {showLightbox && (
                        <div className="fixed inset-0 z-[100] bg-black/95 flex items-center justify-center p-4 animate-fade-in" onClick={() => setShowLightbox(false)}>
                            ...
                            <div className="absolute top-4 right-4 flex gap-4 z-50">
                                <Button variant="primary" onClick={(e) => { e.stopPropagation(); downloadImage(state.currentImage!); }} className="shadow-xl">
                                    Baixar Imagem
                                </Button>
    ...
    return (
        <div ref={resultRef} className="sticky top-20">
            {renderContent()}
            {showGallery && <GalleryModal history={state.history} onClose={() => setShowGallery(false)} onDownload={downloadImage} onDelete={onDelete} />}
        </div>
    );
};
