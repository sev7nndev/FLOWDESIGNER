...
import { Upload, AlertTriangle, CheckCircle2, DollarSign, Settings, Save, Loader2, Link, Unlink, Info, Activity } from 'lucide-react';
...
export const PlanSettingsManager: React.FC = () => {
    ...
    const handleInputChange = (id: UserRole, field: keyof EditablePlan, value: string) => {
        setPlans(prev => prev.map(p => {
            ...
            if (field === 'max_images_per_month') return { ...p, max_images_per_month: parseInt(value) || 0 };
...
export const MercadoPagoManager: React.FC<{ user: User }> = ({ user }) => {
    ...
    const checkConnectionStatus = useCallback(async () => {
        if (!isOwner || !supabase) { setIsLoading(false); return; }
        setIsLoading(true);
        try {
            const { data } = await supabase.from('owners_payment_accounts').select('owner_id').limit(1).maybeSingle();
            setIsConnected(!!data);
        } catch (e) {
    ...
    return (
        <div className="space-y-4 bg-zinc-900/50 p-6 rounded-xl border border-white/10">
            <h3 className="text-xl font-bold text-white border-b border-white/10 pb-2 flex items-center gap-2">
                <DollarSign size={20} className="text-green-500" /> Integração Mercado Pago
            </h3>
            ...
            {isLoading ? (
                ...
                <div className="p-3 bg-green-500/10 border border-green-500/20 text-green-400 text-sm rounded-lg flex items-center justify-between">
                    <span className="flex items-center gap-2"><Link size={16} /> Conta Conectada</span>
                    <Button variant="ghost" onClick={handleDisconnect} className="h-8 text-xs text-red-400 hover:bg-red-500/10" icon={<Unlink size={14} />}>Desconectar</Button>
                </div>
            ) : (
                <div className="p-3 bg-yellow-500/10 border border-yellow-500/20 text-yellow-400 text-sm rounded-lg flex items-center justify-between">
                    <span className="flex items-center gap-2"><AlertTriangle size={16} /> Pagamentos Desconectados</span>
                    <Button onClick={handleConnect} className="h-8 text-xs" icon={<Link size={14} />}>Conectar Agora</Button>
                </div>
            )}
