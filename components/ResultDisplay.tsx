...
// import { GalleryModal } from './Modals'; // Removed GalleryModal import
...
const ResultDisplayComponent: React.FC<ResultDisplayProps> = ({ state, downloadImage, showGallery, setShowGallery }) => {
    ...
    const renderContent = () => {
        ...
        if (state.currentImage) {
            // NOTE: ImageResult expects (url, filename) but here it receives (image). 
            // Since this component is likely deprecated/unused in favor of GenerationHistory, 
            // I will keep the original signature for ImageResult here to avoid cascading changes, 
            // but the main app uses GenerationHistory.
            return (
                <section className="animate-fade-in pt-4">
                    <div className="text-center mb-8">
                        <span className="text-primary text-xs font-bold uppercase tracking-widest mb-2 block">Resultado Final</span>
                        {/* NOTE: Keeping original download signature for ImageResult here */}
                    </div>
