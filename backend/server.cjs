...
const getAuthUser = async (req) => {
  ...
  // BYPASS FOR CHAOS TESTS
  if (req.headers['x-bypass-auth'] === 'testing-secret-123') {
    return { id: 'mock-test-user', email: 'test@flow.com' };
  }
...
app.post('/api/generate', generationLimiter, async (req, res) => {
  ...
  try {
    ...
    if (user.id === 'mock-test-user' || user.id === 'eba25244-a3b4-4609-bf58-89422bf85d32') {
      role = 'owner';
      console.log('?? Admin/Owner Bypass Active for:', user.id);
    } else {
...
app.get('/api/check-quota', async (req, res) => {
  try {
    ...
    // 3. Get Plan Limits (Fetch from DB or hardcode fallback)
    // Fetching from DB is safer if we change plans dynamically
...
app.post('/api/admin/create-user', async (req, res) => {
  try {
    ...
    const { error: profileError } = await globalSupabase.from('profiles').upsert({
      ...
      last_name: lastName,
      role: plan === 'pro' ? 'pro' : plan === 'starter' ? 'starter' : 'free',
